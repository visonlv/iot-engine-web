...
import { ProFormSelect,ProFormInstance,ModalForm, ProFormText,ActionType } from '@ant-design/pro-components';
...
const AddOrUpdateUser: React.FC<{
    ...
    const formSubmit = async (values: any) => {
       const item :API.authUser = {
        ...
        let code = -1  
        if (flag === 'update') {
          const updateReq : API.authUserUpdateReq = {
            app_code:APPCODE,
            Item:item,
          } 
          const res = await updateHandler<UpdateFuncType, API.authUserUpdateReq>(userServiceUpdate, pageRef, updateReq);
          code = res.code
        } else {
          const addReq : API.authUserAddReq = {
            app_code:APPCODE,
            item:item,
          } 
          const res = await addHandler<AddFuncType, API.authUserAddReq>(userServiceAdd, pageRef, addReq);
          code = res.code
        }
        if (code === 0) {
          onClose();
