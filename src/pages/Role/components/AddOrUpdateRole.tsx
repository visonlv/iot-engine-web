...
const AddOrUpdateRole: React.FC<{
    ...
    const formSubmit = async (values: any) => {
      let code = -1  
      if (flag === 'update') {
        const updateReq : API.authRoleUpdateReq = {
          code: values.code,
          name: values.name,
          id: record?.id as string,
        } 
        const res = await updateHandler<UpdateFuncType, API.authRoleUpdateReq>(roleServiceUpdate, pageRef, updateReq);
        code = res.code
      } else {
        const addReq : API.authRoleAddReq = {
          code: values.code,
          name: values.name,
        } 
        const res = await addHandler<AddFuncType, API.authRoleAddReq>(roleServiceAdd, pageRef, addReq);
        code = res.code
      }
      if (code === 0) {
        onClose();
