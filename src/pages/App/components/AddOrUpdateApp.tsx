...
const AddOrUpdateApp: React.FC<{
    ...
    const formSubmit = async (values: any) => {
      let code = -1 
      if (flag === 'update') {
          const updateReq : API.authAppUpdateReq = {
            code: values.code,
            name: values.name,
            describe: values.describe,
            id: record?.id as string
          } 
          const res = await updateHandler<UpdateFuncType, API.authAppUpdateReq>(appServiceUpdate, pageRef, updateReq);
          code = res.code
        } else {
          const addReq : API.authAppAddReq = {
            code: values.code,
            name: values.name,
            describe: values.describe,
          } 
          const res = await addHandler<AddFuncType, API.authAppAddReq>(appServiceAdd, pageRef, addReq);
          code = res.code
        }
        if (code === 0) {
          onClose();
