...
import { apiWhiteListServiceAdd, apiWhiteListServiceUpdate } from '@/services/auth/apiWhiteListService';
...
const AddOrUpdateWhitList: React.FC<{
    ...
    const formSubmit = async (values: any) => {
      let code = -1  
      if (flag === 'update') {
        const updateReq : API.authApiWhiteListUpdateReq = {
          path: values.path,
          name: values.name,
          id: record?.id as string,
        } 
        const res = await updateHandler<UpdateFuncType, API.authApiWhiteListUpdateReq>(apiWhiteListServiceUpdate, pageRef, updateReq);
        code = res.code
      } else {
        const addReq : API.authApiWhiteListAddReq = {
          path: values.path,
          name: values.name,
        } 
        const res = await addHandler<AddFuncType, API.authApiWhiteListAddReq>(apiWhiteListServiceAdd, pageRef, addReq);
        code = res.code
      }
      if (code === 0) {
        onClose();
