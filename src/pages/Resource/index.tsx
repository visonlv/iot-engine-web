...
import { RESOURCE_TYPE_API, RESOURCE_TYPE_MENU, RESOURCE_TYPE_RULE, RESOURCE_TYPE_VALUE } from '@/utils/const';
...
const Page: React.FC = () => {
  ...
  const pageRef = useRef<ActionType>();
    // 删除操作
    const showDeleteConfirm = (record: API.authResource) => {
      const body:API.authResource = {
        id: record.id ?? '',
      };
      deleteHandler<API.authRequest>(resourceServiceDel, pageRef, {
        title: '是否删除当前资源',
   ...
   * 查询数据
   * */
  const queryPage = async (params: any,): Promise<{ data?: API.authResource[]; total?: number }> => {
  ...
  const columns: ProColumns<API.authResource>[] = [
    ...
    },
    {
      ...
      valueType: 'dateTime',
      render: (_, entity: API.authResource) => timestampToDateStr(Number(entity.create_time), 'YYYY-MM-DD HH:mm:ss.SSS'),
      search: false,
    },
    {
      title: '操作',
      valueType: 'option',
      key: 'option',
      render: (text, record: API.authResource) => (
        <>
